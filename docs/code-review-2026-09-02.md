...
> 评审方式：四路并行专项精读（数据安全与核心模型、AI 与远程操控、引擎与服务层、视图/测试/工程卫生），随后对全部 P0/P1 与关键 P2 逐条二次核实（含引擎源码 `ThirdParty/Pikafish` 的阻塞语义、git 对象归属统计等）。未经证实的推测已剔除。
> 二次核实修正：原列为 P1 的「Cmd+Q 不提示」实为 #172 中用户明确选择的工作流，改列为设计决策而非缺陷；「快捷键无冲突检测」降为预防性 P2；「换局重置 showAllNextMoves」是带注释的有意行为，改为可维护性建议；仓库体积归属数字按实测更正。
...
2. **跨设备数据丢失路径仍在**：崩溃恢复快照会被静默删除、恢复备份后版本号不抬高触发反向覆盖。
...
`ViewModel.swift:1938`（`aiRespondIOS` 只 `guard !isEvaluatingIOS`）、`ViewModel.swift:1871`（问棋侧只查 `isEvaluatingIOS`，反向无守卫）、`PikafishServiceIOS.swift`（自身无任何忙碌守卫，注释把互斥责任推给 ViewModel）、`PikafishEngineBridge.mm:188-222`。

互斥是单向的：问棋 `evaluate`（默认 3 s）进行中，用户点「AI应招」会对同一个 `Stockfish::Engine` 再次 `setPosition` + `go`。后果链（已按引擎源码核实：`Engine::go` → `ThreadPool::start_thinking` 第一行即 `main_thread()->wait_for_search_finished()`，`thread.cpp:285`）：

- 第二次 `go` 在主线程阻塞直到第一次搜索结束（最长 3 s），界面冻结；
- `set_on_bestmove` 无锁覆盖搜索中的回调；第一次搜索完成调用的是**第二个**回调 → 第一个 `withCheckedContinuation` 永不恢复，`isRemoteAnalyzing` 的 `defer` 永不执行，之后所有问棋恒 ENGINE_BUSY；
...
`ViewModel.swift:1783-1815`（`pikafishQuickMove` 查 `queue.isIdle` 后直接调 `service.evaluatePosition`，不入队）、`EvaluationQueue.swift:60-69`（`enqueue` 不检查快速应招/远程分析在飞）、`PikafishService.swift:365-406`。
...
`PikafishService.swift:351-355`（`sendCommand` 无 `isRunning` 检查，用旧式 `FileHandle.write(_:)`）、`:267`（`stopCurrentSearch`）、`:247`（`analyzePosition` 的 `defer` 在 `engineTerminated` 抛出后仍写管道）。全仓库无 `signal(SIGPIPE, SIG_IGN)`。

场景：引擎崩溃或因 NNUE 校验失败自行 `exit()`（`network.cpp:160`），用户点「取消评估」→ `cancelAll` → `stopCurrentSearch` → 向已关闭管道 `write` → SIGPIPE。即便忽略了信号，`FileHandle.write(_:)` 在 EPIPE 时抛 ObjC 异常，Swift 同样接不住。
...
### 1.5 恢复备份 / 恢复快照后 dataVersion 不抬高，触发反向覆盖链 【P1】

`Database.swift:340-363`、`ViewModel.swift:389, 1589`、`Session.swift:377`（`currentCheckpointDataVersion = dirty ? dataVersion - 1 : dataVersion`）。`restoreFromBackup` 直接采用备份文件的 `dataVersion` 并置脏，不与远端比较（上一轮 1.4 后半仍开放）。

场景：iCloud 存档 v500，本机恢复 v100 备份 → checkpoint 99 → `saveToDefault` 弹「别处被修改过」→ 用户确认强存 → 文件变 v100；另一设备收到变更后走 `reloadFromRemote`，`remoteVersion(100) <= currentVersion(500)` 被忽略（`ViewModel.swift:1461`），它下次保存又把恢复结果覆盖回去。

**修复**：恢复时 `dataVersion = max(backup, loadDataVersionFromDefault() ?? 0) + 1`。

### 1.6 问棋取消后 `wireMessages` 被污染，且弹假错误 【P1】

`ChatViewModel.swift:243-247`、`:216-221`、`:186`。`cancel()` 立即 `rollbackToLastUserMessage()`，但正在 `await toolbox.execute(...)`（引擎分析，可达十几秒）的循环体返回后不检查 `Task.isCancelled`，直接 `wireMessages.append(.toolResult(...))`——此时对应的 assistant tool_calls 消息已被回滚删掉，留下一条孤儿 tool 消息，下一次提问会被 OpenAI 兼容服务以「tool 消息无配对 tool_calls」拒绝。同一位置，`client.send` 因 Task 取消抛出的 `URLError.cancelled` 落入 `catch`，被 `LLMClient.transportError`（`:327-336`，非超时一律 `.network`）映射后 `setError`，用户点「停止」后看到「连不上服务」红条加重试按钮。`ChatViewModelTests` 无任何取消场景覆盖。

**修复**：`await` 之后先 `guard !Task.isCancelled else { return }` 再 append；catch 里 `if Task.isCancelled { return }`。

### 1.7 旧 Task 的收尾会覆盖新一轮的运行状态 【P1】
...
### 1.8 多窗口下键盘事件串到错误的 ViewModel 【P1】

`XiangqiNotebookApp.swift:9` 用 `WindowGroup`，全仓库没有 `CommandGroup(replacing: .newItem)`，Cmd+N 会开第二个窗口；`MacContentView.swift:7-17` 每个窗口各建一个 `@StateObject ViewModel`（以及各自的 `RemoteControlServer`）；`:223-266` 每个窗口各装一个 `NSEvent.addLocalMonitorForEvents`，回调只检查 `NSApp.keyWindow?.firstResponder`，从不核对 `event.window`。
...
### 1.9 路径编辑器 indices + Binding 删除后可能越界 【P1，未实机复现】

`MacPathEditorView.swift:197-214`：`ForEach(pathGroups.indices, id: \.self)` 内构造 `Binding { pathGroups[index] }`，而 `onDelete` 里 `pathGroups.remove(at: index)`；`72-102` 行的 `paths[pathIndex]` 同款。这是 SwiftUI 的已知崩溃模式（删除末尾元素后旧行的 getter 可能再被求值一次），行内还有绑定到 `pathGroup.name` 的 TextField 加大了概率。上一轮修的是 stale `selectedGroupIndex`，这是另一条路径。本轮只做了代码核实，未实机复现。

**修复**：ForEach 直接绑 `$pathGroups`（Identifiable），或 getter 用 `indices.contains(index)` 兜底。

---

## 二、设计决策确认（非缺陷，记录以免反复讨论）

- **Cmd+Q 不提示、直接丢弃未保存改动，并清除崩溃恢复快照**（`ViewModel.swift:267-276`）。这是 #172 中按用户反馈明确保留的工作流（「保留不自动保存、Cmd+Q 即丢弃未保存改动」），快照只用于崩溃/被杀场景。本轮不视为缺陷。需注意它与 1.4 叠加：只有崩溃路径能留下快照，而 1.4 又会在跨设备场景把这份快照静默清掉，所以 1.4 值得优先修。
- **`setFilters` 在非复习/练习模式强制 `showAllNextMoves = true`**（`SessionManager.swift:115-121`）。带注释的有意行为（「其他情况默认打开路径和下一步」），与 356233b 修的「漏拷字段」不是一回事。可维护性建议见 2.4。
...
- **被中途 stop 的半截分析结果按「足额 movetime」入缓存并同步到 iPhone**。`ViewModel+AnalysisToolHost.swift:58-88`：`remoteEngineAnalyze` 返回后不分青红皂白以 `movetimeMs: movetime` 调 `setEngineAnalysis`；`ChatViewModel.cancel()` 调 `stopRemoteEngineAnalyze()` 让引擎提前吐 `bestmove`，`flushAnalysisCache` 在取消路径也照样落盘（注释说这是有意为之）。之后 `findUsableEngineAnalysis` 把 0.5 秒的结果当 5 秒结论秒回，且标 `fromCache: true`。修复：`analyzePosition` 返回是否被 stop / 实际耗时，被中断的不入缓存。
- **`ensureFenId` / `ensureMove` 用 `count + 1` 发号**（`DatabaseView.swift:57, 123`）。目前全仓库对 `fenObjects2`/`moveObjects` 没有任何物理删除（move 只软删除置 `targetFenId = nil`），所有创建都走这两处，因此 id 实际连续、暂无风险；`addFenObjectIfNeeded`（`:525`）可写任意 id 但无生产调用方。属防御性改动：`(keys.max() ?? 0) + 1` 一行即可，预防未来任何删除/合并逻辑引入静默覆盖。
- **保存前版本校验的漏洞与 TOCTOU**。`ViewModel.swift:1330-1370`、`Database.swift:33,44`。(a) `loadFailedAtStartup` 被设置但全仓库无人读取；存档 `data_version` 字节扫描成功但整体解码失败时，启动是空库，保存走「别处被修改过」分支——有确认框但不做自动备份、文案误导。(b) 版本检查 → 用户弹窗 → `saveAsync` 之间无二次校验，异步化后窗口更宽。修复：`loadFailedAtStartup` 为真强制走备份分支；确认后写盘前再读一次远端版本。
...
- **`Database.reload()` 仍是异步生效**。`Database.swift:174-186`。`reloadFromRemote`/`checkDataVersion` 在 `try reload()` 返回后立刻弹「数据已更新」，此时 `databaseData` 还没换；`showConflictAlert` 里 `reloadFromRemote()` 后紧跟 `setDataClean()`（`ViewModel.swift:1535-1538`）顺序依赖 runloop。修复：与 `markDirty` 同样改为 `runOnMain` 同步。
- **CourseImportService（新）整体正确**，两点小问题：`ViewModel.swift:2696` `Int(seconds.rounded())` 对超范围 Double 会 trap，`times` 来自外部 JSON 未校验，建议 clamp；各线路 `startFen` 可不同但 `startingFenId` 只取第一条（`CourseImportService.swift:94-96`），其余线路的边会进入 `moveIds` 却从起点不可达，建议要求所有线路同一 startFen 否则抛 `invalidLine`。
- **`Session.currentFenObject` 的 `fatalError` 仍在**（`Session.swift:110`）。启动路径有校验不可达，运行期数据不一致仍会杀进程。

### 3.2 引擎与服务

- **`willTerminate` 里的引擎清理是死代码**。`ViewModel.swift:271-276` 观察者内再开 `Task { @MainActor }` 去 `cancelAll()`/`stop()`，而 `applicationWillTerminate` 返回后 AppKit 直接 `exit()`，主线程不会再转一圈。目前靠 pikafish 读到 stdin EOF 自动 `quit` 兜底。修复：观察者已在主队列，直接同步调用。
- **iOS `releaseResources` 并不释放内存**。`PikafishServiceIOS.swift:104-111` 注释称「释放置换表与线程池内存」，但 `Engine::search_clear`（`engine.cpp:134-139`）只是 `tt.clear` 置零并清历史表，64 MB Hash 与线程池全部保留。修复：改为 `setHashMB(1)`/`setThreads(1)` 再 resize，或删掉误导性注释。
- **NNUE 缺失/损坏时 iOS 进程内 `exit(EXIT_FAILURE)`**。`PikafishServiceIOS.swift:40-42` 找不到资源静默跳过 `loadNetwork`，随后 `go` → `verify_networks` 直接 `exit()`（`network.cpp:160`）。修复：`nnuePath == nil` 时直接返回错误，不调用 `go`。
...
### 3.3 远程操控 / MCP / Claude 桥接

- **RemoteControlServer 请求体无上限，鉴权发生在读完整个 body 之后**。`RemoteControlServer.swift:117-176`。`receiveHTTPRequest` 无限累加；任何本机进程不带 token 也能让 app 吞任意大 body 后才收到 403。`String(data:encoding:.utf8)` 对非法 UTF-8 恒返回 nil → `hasCompleteHTTPRequest` 恒 false，连接挂到对端主动关闭。token 比较用 `==` 非常量时间（本机模型下影响有限）。修复：累计超过 1 MB 即回 413 并 cancel；头部一到齐就先校验 token。
...
- **token 文件权限两边不一致**。`RemoteControlServer.swift:54`（`String.write(to:atomically:)` 默认 0644）vs `mcp/claude-bridge.mjs:402`（0o600）。修复：写后 `setAttributes([.posixPermissions: 0o600])`。
...
- **桥接在响应关闭时过早释放飞行槽**。`mcp/claude-bridge.mjs:339-349`。`res.on("close")` 里 `cleanup()` 把 `activeChild = null`，而子进程此刻才刚收到 SIGTERM（最长 5 秒宽限）；`:244` 的忙碌检查随即放行下一个 `/chat`，旧 claude 未退出时再 spawn 一个，两者都去抢 9214 的引擎。修复：`activeChild` 只在 `child.on("close"|"error")` 里清。
...
- **快捷键注册无冲突检测**（预防性）。`ActionDefinitions.swift:325, 352` `shortcutLookup[shortcutKey] = key` 静默覆盖；`0b27345` 修过 `,l` 重复。ViewModel 里 95 处注册全靠人工保证唯一（本次逐一核对过，`,v` 由 `#if os` 隔开，当前无冲突）。修复：注册时 `assert(shortcutLookup[shortcutKey] == nil, ...)`，并在 `ActionDefinitionsTests` 加一条全量唯一性测试。
- **`setFilters` 逐字段手抄 SessionData 的可维护性**。`SessionManager.swift:106-130` 新建 `SessionData()` 后逐字段拷贝，356233b 已因漏拷修过一次，新增字段仍会再漏。建议给 `SessionData` 加 `copy()`（Codable 往返即可），setFilters 先整体拷贝再覆盖需要重置的字段。
...
- **`BoardViewModel.getCurrentTurn` 硬下标**。`BoardViewModel.swift:121-124` `components[1]`，FEN 无空格即崩（`Board.swift:351` 点击棋子时调用）。目前入库有校验，但 `/apply` 等外部 FEN 入口在增多。修复：`components.count > 1 ? ... : "r"`。
...
- `Views/DesignTokens.swift` 全为浅色固定值，无深色适配。
...
- 仓库 pack 约 100 MiB，主因不是 nnue：`refs/codex/turn-diffs/checkpoints/...` 两个 ref 独占约 98 MiB、3749 个对象（其中 3386 个是 `DerivedDataScreenshot/` 截图），不在任何分支上（实测：`git rev-list --objects <codex refs> --not --branches --tags`）。删 `refs/codex/*` 后 `git gc --prune=now` 即可瘦身。
...
## 六、上一轮（2026-06-10）问题状态核查

| 项 | 状态 | 证据 |
|---|---|---|
| 1.1 解码失败→空库→无确认覆盖 | 部分修复 | `Database.swift:38-50` 区分文件存在/不存在；`ViewModel.swift:1349-1367` 版本读不出时确认+自动备份；但 `loadFailedAtStartup` 无人读取，字节扫描能读出版本时走普通确认分支、无备份（见 3.1） |
| 1.2 无自动保存 | 按 #172 改为崩溃快照 | macOS 30 s 快照 `ViewModel.swift:333-364`；iOS 仅进后台时写 `:287-303`，前台崩溃仍全丢；Cmd+Q 不提示为用户有意选择（见 二）；跨设备后快照被静默丢弃（1.4） |
| 1.3 markDirty 竞态 | 已修复 | `Database.swift:77-88` guard 与自增同在 `runOnMain` 临界区；`saveAsync` 用 `mutationCount` 区分保存期间新修改 |
| 1.4 iCloud 冲突 | 部分修复 | `iCloudFileCoordinator.swift:98-146` 按 mtime 取新、经写协调、置 `isResolved`；恢复备份版本号问题仍开放（1.5）；输方版本仍永久删除（3.1） |
| 1.5 保存抑制窗口 | 已修复 | `iCloudFileCoordinator.swift:242-255` 窗口结束比 mtime 补发通知 |
| 1.5 引擎分数合并 | 基本修复 | `EngineScoreStorage.swift:67-102` 按 fenId 合并；占位文件未下载时仍会覆盖（3.1） |
| 1.5 saveToDefault TOCTOU | 仍开放 | `ViewModel.swift:1330-1370`，异步化后窗口更宽 |
| 1.5 备份/恢复错误只 print | 仍开放 | `ViewModel.swift:1563-1566, 1594` |
| 崩溃#1 MacPathEditorView 索引 | 已修复（另有新路径） | `MacPathEditorView.swift:107-112, 168-172, 215-219, 260`；indices + Binding 见 1.9 |
...
2. **跨设备数据丢失两件套**（1.4、1.5）：改动都很小，但都是「用户以为已恢复/已保存，实际被另一设备静默覆盖」。
3. **问棋取消**（1.6、1.7）并补取消场景测试。
