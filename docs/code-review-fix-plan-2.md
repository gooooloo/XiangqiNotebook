...
- **阶段一 8 个 bug 全部修复**：#127、#155、#157、#156、#158、#159、#160、#128，各一个独立 commit。
- **阶段二完成**：#166、#165、#161 全部完成；#162 以新的 `GamePathEnumerator`（计数式组合枚举）完成；#164 完成（session 收紧为 private + 三处违规收敛）。
- **部分完成（issue 保持开放并已留言说明）**：
  - #163：完成紧凑编码与版本号字节级扫描；「编码与写入后台化」涉及 DatabaseData 可变引用与后台编码的竞态，需不可变快照设计，留待后续。
  - #167：完成伪测试修复、《适情雅趣》导入测试开关、组合视图补测 10 个；共享 TestDatabaseBuilder 与 iCloud 链路测试留待后续。
  - #168：完成 Utils.swift 死代码删除；视图层重复消除与 GameBrowserView 拆分需逐屏视觉验证，不适合无人值守执行，留待后续。
- 每个 commit 均通过完整 macOS 测试套件；涉及共享/iOS 代码的节点经 iOS Simulator 构建验证。
